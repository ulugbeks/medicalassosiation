...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route('portfolio.update', $portfolio->id) }}" method="POST">
                            ...
                            <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" 
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" 
                                           ...
                                           value="{{ old('title.'.$language->code, $portfolio->getTranslation('title', $language->code, false)) }}" 
                                ...
                                <div class="form-group mt-3">
                                    ...
                                    <input type="text" 
                                           ...
                                           value="{{ old('category.'.$language->code, $portfolio->getTranslation('category', $language->code, false)) }}" 
                                           {{ $language->is_default ? 'required' : '' }}>
                                ...
                                <div class="form-group mt-3">
                                    ...
                                    <textarea name="description[{{ $language->code }}]" 
                                              ...
                                              rows="3">{{ old('description.'.$language->code, $portfolio->getTranslation('description', $language->code, false)) }}</textarea>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="active" id="active" class="form-control @error('active') is-invalid @enderror">
                            <option value="1" {{ old('active', $portfolio->active) == '1' ? 'selected' : '' }}>{{ __('Active') }}</option>
                            <option value="0" {{ old('active', $portfolio->active) == '0' ? 'selected' : '' }}>{{ __('Inactive') }}</option>
                        </select>
                        @error('active')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
            ...
            <div class="mt-4">
                <button type="submit" class="btn btn-primary">{{ __('Update Portfolio Item') }}</button>
